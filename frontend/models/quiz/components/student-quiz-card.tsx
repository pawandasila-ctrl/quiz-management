    ...
    <Card className="border-border shadow-sm flex flex-col justify-between overflow-hidden">
      ...
      <CardFooter className="p-3 flex flex-col gap-2">
        {activeAttempt ? (
          ...
          <Link
            href={`/dashboard/quiz/${quiz.id}/attempt/${latestAttempt?.id}`}
            className="w-full inline-flex items-center justify-center rounded-md border border-border bg-background px-3 text-xs font-medium text-foreground shadow-sm hover:bg-accent transition-colors h-8.5 gap-1.5"
          >
