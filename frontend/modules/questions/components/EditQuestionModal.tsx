...
import { useUpdateQuestion } from "@/modules/admin/hooks";
import { Question, QuestionType } from "@/modules/quiz/types";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function EditQuestionModal({
  ...
  const [qType, setQType] = useState<QuestionType>(question.type);
  const [qMarks, setQMarks] = useState(question.marks);
  const [qOrder, setQOrder] = useState(question.order);
  const [qExplanation, setQExplanation] = useState(question.explanation || "");
  const [qImageUrl, setQImageUrl] = useState(question.image_url || "");
  ...
  const [mcqOptions, setMcqOptions] = useState(() => {
    if (question.type === "mcq") {
      ...
      while (opts.length < 4) {
        opts.push({ text: "", is_correct: false, order: opts.length + 1 });
      }
      return opts;
    }
    return [
  ...
  const [tfCorrect, setTfCorrect] = useState<"true" | "false">(() => {
    if (question.type === "true_false") {
      ...
      return correctOpt?.text.toLowerCase() === "true" ? "true" : "false";
    }
    return "true";
  });
